...
def parse_results(results: list, flatten: bool = False) -> list:
    """
    Defensive parsing of results coming from pymongo.

    - Logs incoming row types/preview to help diagnose issues.
    - If a row is a dict: parses normally.
    - If a row is a JSON string: attempt to json_loads and parse.
    - If a row is a list/tuple: wraps into a 'value' field.
    - For other types: wraps into a 'value' field (stringified).
    - Never throws 'string indices must be integers'.
    """
    rows = []
    columns = []

    for row in results:
        # Debug: type and short preview to identify problematic rows
        try:
            logger.debug("parse_results: incoming row type=%s preview=%s", type(row), str(row)[:400])
        except Exception:
            logger.debug("parse_results: incoming row type=%s (preview failed)", type(row))

        parsed_row = {}

        # If it's a dict, parse it (normal case)
        if isinstance(row, dict):
            parsed_row = _parse_dict(row, flatten)

        # If it's a string, try to interpret as JSON/document
        elif isinstance(row, str):
            try:
                decoded = json_loads(row, object_hook=datetime_parser)
                if isinstance(decoded, dict):
                    parsed_row = _parse_dict(decoded, flatten)
                else:
                    # decoded to a scalar or list -> wrap it
                    parsed_row = {"value": decoded}
            except Exception:
                # Not JSON - keep raw string in a value field
                parsed_row = {"value": row}

        # If it's a list/tuple -> wrap into a 'value' field
        elif isinstance(row, (list, tuple)):
            parsed_row = {"value": list(row)}

        # Otherwise, try to stringify to keep things consistent
        else:
            try:
                parsed_row = {"value": str(row)}
            except Exception:
                parsed_row = {}

        # Register columns and their types
...
class MongoDB(BaseQueryRunner):
    ...
    def __init__(self, configuration):
        ...
        # FORCE flattening OFF to avoid 'string indices must be integers' errors
        self.flatten = False
        logger.debug("flattening forced OFF")
    ...
    def run_query(self, query, user):  # noqa: C901
        ...
        if q or (not q and not aggregate):
            ...
            try:
                ...
                return None, f"MongoDB aggregation error: {str(e)}"

            # Backwards compatibility with older pymongo versions that returned a dict with "result"
            if isinstance(r, dict):
                ...
                logger.debug("✅ MongoDB aggregation output type: %s", type(cursor))
                logger.debug("✅ MongoDB aggregation output sample: %s", str(cursor)[:500])
                logger.debug("✅ Cursor before parsing: %s", type(cursor))
        ...
        if "count" in query_data:
            ...
            # Defensive parsing: we pass flatten explicitly (default off in __init__)
...
#     for row in results:
#         # Ensure that the row is always a dictionary to prevent string index errors
#         if not isinstance(row, dict):
#             row = {}

#         # Always parse without flattening, keep arrays intact
#         parsed_row = _parse_dict(row, False)
...
#             rows.append({"count": cursor})
#         else:
#             rows, columns = parse_results(cursor, flatten=False)
